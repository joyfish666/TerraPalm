...
export class HandTracker {
    ...
    _processResults(results) {
        ...
        const prevHandCount = this._lastHandCount || 0;
        const currentHandCount = results.landmarks ? results.landmarks.length : 0;
        ...
        if (currentHandCount > 0) {
            ...
            // 手部数量变化时输出日志
            if (prevHandCount !== currentHandCount) {
                console.log(`[HandTracker] 检测到 ${currentHandCount} 只手`);
            }
        } else {
            // 从有手变为无手时输出日志
            if (prevHandCount > 0) {
                console.log(`[HandTracker] 手部丢失`);
            }
        }

        this._lastHandCount = currentHandCount;
