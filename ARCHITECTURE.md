...
| 手势 | 手部 | 动作 | 控制输出 |
|------|------|------|----------|
| 张开手掌 + 移动 | 左手 | 平移地形 | `panX`, `panZ` |
| 张开手掌 + 左右摆动 | 右手 | 旋转地形 | `rotateY` |
| 拇指+食指 捏合/张开 | 右手 | 缩放地形 | `zoom` |
| 握拳 | 任意手 | 暂停该手控制 | 无输出 |

**手势检测算法**：
- **张开手掌判断**：比较指尖（8,12,16,20）与 MCP 关节（5,9,13,17）的 Y 坐标，至少 3 根手指伸直即为张开
- **捏合距离**：计算拇指尖（4）和食指尖（8）的欧氏距离
- **手掌中心**：手腕（0）和中指 MCP（9）的中点

**速度计算**：
- 保留最近 5 帧的位置历史
- 计算平均速度作为控制输入
- 速度阈值 0.001（低于此值忽略，防止漂移）

**缩放状态机**（防止松手误触）：
```
手指距离 < 0.06  →  捏合状态（PINCHING）
手指距离 0.06-0.12 →  死区（NEUTRAL，忽略）
手指距离 > 0.12  →  张开状态（SPREADING）
```
- 只有在同一状态内的持续变化才触发缩放
- 状态切换时忽略（300ms 冷却时间）
- 防止"捏合后松手"被误判为"放大"
