...
| 手势 | 手部 | 动作 | 控制输出 | 识别方式 |
|------|------|------|----------|----------|
| 张开手掌移动 | 左手 | 地形平移 | `panX`, `panZ` | 左手张开 + 移动 |
| 张开手掌左右移动 | 右手 | 绕中心轴旋转 | `rotateY` | 右手张开 + 左右摆动 |
| 双手向外推开 | 双手 | 放大 | `zoom` | 双手掌心朝摄像头 + 间距增大 |
| 双手向内收拢 | 双手 | 缩小 | `zoom` | 双手掌心朝面部 + 间距缩小 |

**双手缩放机制**：
- 检测到双手同时做出缩放手势时，**锁住**左手平移和右手旋转
- 双手必须都张开才触发缩放
- 通过双手中心距离变化速度判断缩放方向
- 锁定后 500ms 无操作自动解除

**掌心方向检测**：
- 使用手掌宽度（食指MCP到小指MCP）与手掌深度（手腕到中指MCP）的比值
- 比值 > 0.5 → 掌心朝向摄像头（手指展开）
- 比值 ≤ 0.5 → 掌心朝向面部（手指重叠）
